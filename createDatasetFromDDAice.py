...
if len(ground_est0) == 1:
	...
	vario_data_ge = run_vario(ground_est0, lag, winsize, winstep, nvar, ndir, nres)
	vario_data_wp = run_vario(weight_photons0, lag, winsize, winstep, nvar, ndir, nres, True)
else:
	# compute variograms for each track individually, then combine results
	ge_dat, wp_dat = [],[]
	for data in ground_est0:
		ge_dat.append(run_vario(data, lag, winsize, winstep, nvar, ndir, nres))
	vario_data_ge = np.vstack((ge_dat))

	for data in weight_photons0:
		wp_dat.append(run_vario(data, lag, winsize, winstep, nvar, ndir, nres, True))
