...
label_cmap = matplotlib.colormaps.get_cmap('tab20')
plot = Image.open('calipso_690_4998.png') #for multiple images have [self.tiff_selector] inside ()
tiff_image_matrix = np.array(plot)
selected_classes = np.ones(4)
split_disp_size = [25,25]
pred_label_path = 'Output/calipso_config.config_14-02-2025_16:36/labels/labeled_epoch_50.npy'
...
for i, selected_class in enumerate(selected_classes):
        if selected_class:
            ...
            for splitImg in xy:
                ...
                y_end = min(split_disp_size[1] + y_start, img_mat.shape[0])
...
plt.imshow(img_mat,origin='lower')
plt.show()
