...
model = DDAiceNet2.DDAiceNet2(num_classes, nres*2, hiddenLayers=hidden_layers)
...
# print(dataset['info'])
# print(dataset['labs'].shape)
# print(dataset['conf'].shape)
# print(dataset['ll'].shape)
# print(dataset['hist'].shape)

dataset_info = dataset['info']
labels = dataset['labs']
dataset_coords = dataset['ll'][labels != -1]
dataset_labeled = dataset['hist'][labels != -1]
labels = labels[labels != -1]
	
train_size = int(cfg['train_test_split'] * dataset_labeled.shape[0])

train_indices = np.random.choice(range(dataset_labeled.shape[0]), train_size, replace=False)
test_indices = list(set(range(dataset_labeled.shape[0])) - set(train_indices))

train_data = dataset_labeled[train_indices, :]
test_data = dataset_labeled[test_indices, :]

train_labels = labels[train_indices]
test_labels = labels[test_indices]

train_coords = dataset_coords[train_indices, :]
test_coords = dataset_coords[test_indices, :]
...
if weighted:
	with warnings.catch_warnings():
		...
		print('Class 4: {}'.format(y.count(4.0)))
