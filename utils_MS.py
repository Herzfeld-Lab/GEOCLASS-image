...
def draw_split_image_labels(img_mat, scale_factor, split_disp_size, labels, selected_classes, cmap, MS):
    if MS:
...
def draw_split_image_confs(img_mat, scale_factor, split_disp_size, labels, selected_classes, cmap, MS):
    if MS:
        for i,selected_class in enumerate(selected_classes):
            if selected_class:
                clas = labels[labels[:,6] == i]
...
def getImgPathsPan(topDir, recursive=True):
    """
    Return pan_paths found under scene subfolders.
    Uses band count to separate MS (>1 bands) vs pan (1 band).
    """
    pan_paths = []
    scene_dirs = sorted([d for d in glob.glob(os.path.join(topDir, '*')) if os.path.isdir(d)])
    for scene in scene_dirs:
        tifs = sorted(glob.glob(os.path.join(scene, '*.tif')))
        for p in tifs:
            try:
                ...
                # Default to pan if we cannot read band count
                pan_paths.append(p)
    return pan_paths

def getImgPathsMS(topDir):
    """
    Return pan_paths found under scene subfolders.
    Uses band count to separate MS (>1 bands) vs pan (1 band).
    """
...
def silas_directional_vario(img, numLag = 53, lagThresh = 1):
    """
    Implements the directional vario function in python. The variogram is computed
    in 4 different directions: North/South, East/West, and the two diagonals. The
    results are concatenated together as the rows of a numpy array and returned.
...
bg_UTM_path:        {}
        '''.format(yaml_obj['model'],
                   ...
                   yaml_obj['conversion_factor'],
                   yaml_obj['bands'],
                   yaml_obj['wri_green_band'],
                   yaml_obj['wri_red_band'],
                   yaml_obj['wri_nir_band'],
                   yaml_obj['wri_mir_band'],
