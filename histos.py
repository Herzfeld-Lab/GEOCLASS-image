...
def generate_histo(data, hlen, vlen, avg_ht, v_factor):
...
def make_histo_features(ge, wp, windows, lag, vbin_size, v_window_size, nres):
	...
	for w in range(len(windows)):
		...
		histo = generate_histo(wp_data[wp_window_bool], hlen=lag, vlen=vbin_size, avg_ht=int(avg_ht), v_factor=int(v_window_size/2))
