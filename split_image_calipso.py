...
class SplitImageTool(QWidget):
    ...
    def initDataset(self):
        ...
        self.plot = Image.open(self.dataset_info['filename']) #for multiple images have [self.tiff_selector] inside ()
 ...
 #CST20240403 Checks all directories for the image, and deletes it if it finds the image.
    ...
    def makeTiffSelectorCallbacks(self, tiff_num):
        def tiff_selector_callback():
            self.split_info_save = self.split_info #inclued [self.split_info_save[:,6] == self.tiff_selector] if using multiple plots
