...
from auto_rotate_geotiff_MS import *
#CST20240312
...
class SplitImageTool(QWidget):
    ...
    def initDataset(self):
        ...
        self.win_sizeMS = self.dataset_info['MS_winsize_pix']
        if len(self.split_info) > 0:
            centers = self.split_info[:,0:2].astype(np.float64)
            centers[:,0] += self.win_size[0] / 2
            centers[:,1] += self.win_size[1] / 2
        ...
        self.geotiff_ms = rio.open(self.dataset_info['filename'][self.tiff_selector])
        self.tiff_image_matrix_ms = self.geotiff_ms.read()
        self.geotiff_pan = rio.open(self.dataset_info['filename'][self.tiff_selector+1])
        self.tiff_image_matrix_pan = self.geotiff_pan.read(1)
        ...
        if self.cfg['contour_path'] != 'None':
            ...
            self.contour_np = get_geotiff_bounds(self.geotiff_pan, self.utm_epsg_code)
        self.contour_polygon = Polygon(self.contour_np)

        

        self.tiff_image_max_ms = get_img_sigma(self.tiff_image_matrix_ms[::10,::10])
        self.tiff_image_max_pan = get_img_sigma(self.tiff_image_matrix_pan[::10,::10])

        # Precompute MS pixel coords from UTM for display alignment
        self.split_info_ms = self._labels_to_ms_pixels(self.split_info)
        if self.predictions:
            self.pred_labels_ms = self._labels_to_ms_pixels(self.pred_labels)
        else:
            self.pred_labels_ms = None

    def _labels_to_ms_pixels(self, labels):
        if labels is None or len(labels) == 0:
            return labels
        xs = labels[:,4].astype(np.float64)
        ys = labels[:,5].astype(np.float64)

        # Transform UTM -> MS CRS if needed
        if self.geotiff_ms.crs is not None:
            utm_crs = CRS.from_epsg(self.utm_epsg_code)
            ms_crs = self.geotiff_ms.crs
            if ms_crs != utm_crs:
                transformer = Transformer.from_crs(utm_crs, ms_crs, always_xy=True)
                xs, ys = transformer.transform(xs, ys)

        # Affine inverse: (col, row) from (x, y)
        cols, rows = (~self.geotiff_ms.transform) * (xs, ys)

        out = labels.copy()
        out[:,2] = np.round(rows).astype(np.int32)
        out[:,3] = np.round(cols).astype(np.int32)
        return out
    ...
    def initUI(self):
        ...
        self.class_buttons_widget = QWidget(self)
        self.class_buttons_widget.setObjectName("class_buttons_widget")
        self.class_buttons = QVBoxLayout(self.class_buttons_widget)
        ...
        self.visualization_save_buttons.addWidget(self.change_MS_button)
        if self.has_contour == False:
        ...
        filenames = self.dataset_info['filename']
        pair_count = len(filenames) // 2
        for button_idx in range(pair_count):
            tiffNum = button_idx * 2
            button = QPushButton('{}...'.format(filenames[tiffNum].name[:13]), self)
        ...
        self.left_layout.addWidget(self.class_buttons_widget)
        self.class_buttons_index = self.left_layout.count() - 1
    ...
    def addClassButton(self, i, className, container):
        ...
        labelButton = QPushButton('{}: {}'.format(i, className), self.class_buttons_widget)
        ...
        labelToggle = QCheckBox(self.class_buttons_widget)
        labelToggle.toggle()
    ...
    def initBgImage(self):
        if self.isMulti:
            # Original shape: (bands, rows, cols)
            bands, rows, cols = self.tiff_image_matrix_ms.shape
            scale_factor = max(1, int(rows / 1200))
            # Downsample
            bg_img_scaled = self.tiff_image_matrix_ms[:, ::scale_factor, ::scale_factor]
            # Convert to display format (H, W, bands)
            bg_img_scaled = np.moveaxis(bg_img_scaled, 0, -1)
            # Select RGB bands 
            if bg_img_scaled.shape[2] >= 3:
                bg_img_scaled = bg_img_scaled[:, :, [self.r, self.g, self.b]]  # or choose specific bands
            else:
                # If somehow <3 bands
                bg_img_scaled = np.repeat(bg_img_scaled, 3, axis=2)
            # QGIS-like per-band percentile stretch
            # bg_img_scaled = cv2.normalize(bg_img_scaled, None, 0, 255, cv2.NORM_MINMAX)
            # bg_img_scaled = bg_img_scaled.astype(np.uint8)
            bg_img_scaled = stretch_rgb_percentile(bg_img_scaled, 2, 98)
            split_disp_size = (np.array(self.win_sizeMS) / scale_factor).astype('int') - 1
            ...
            self.bg_img_cv, self.bg_img_utm, self.bg_img_transform = rotate_and_crop_geotiff(
                self.dataset_info, self.geotiff_ms, bg_img_scaled, self.utm_epsg_code, self.contour_np, self.tiff_selector
            )
    ...
    #CST 20240312
            ...
            bg_img_scaled = self.tiff_image_matrix_pan[::scale_factor,::scale_factor]
            # QGIS-like per-band percentile stretch for pan
            # bg_img_scaled = scaleImage(bg_img_scaled, self.tiff_image_max_pan)
            bg_img_scaled = stretch_band_percentile(bg_img_scaled, 2, 98)
            ...
            # Rotate tiff to align North and plot glacier contour
            if self.isMulti:
                self.bg_img_cv, self.bg_img_utm, self.bg_img_transform = rotate_and_crop_geotiff(self.dataset_info, self.geotiff_ms, bg_img_scaled, self.utm_epsg_code, self.contour_np, self.tiff_selector)
            else:
                self.bg_img_cv, self.bg_img_utm, self.bg_img_transform = rotate_and_crop_geotiff(self.dataset_info, self.geotiff_pan, bg_img_scaled, self.utm_epsg_code, self.contour_np, self.tiff_selector)
            #height,width,_ = bg_img_scaled.shape
            height,width,_ = self.bg_img_cv.shape
 ...
 #CST20240403 Checks all directories for the image, and deletes it if it finds the image.
    ...
    def _utm_to_pan_pixel(self, x, y):
        if self.geotiff_pan.crs is not None:
            utm_crs = CRS.from_epsg(self.utm_epsg_code)
            pan_crs = self.geotiff_pan.crs
            if pan_crs != utm_crs:
                transformer = Transformer.from_crs(utm_crs, pan_crs, always_xy=True)
                x, y = transformer.transform(x, y)
        col, row = (~self.geotiff_pan.transform) * (x, y)
        return row, col
    ...
    def mousePressEvent(self, event):
        ...
        if button == 1:
            if Point(click_pos_utm[0], click_pos_utm[1]).within(self.contour_polygon):
                if self.lookup_tree is not None:
                    row, col = self._utm_to_pan_pixel(click_pos_utm[0], click_pos_utm[1])
                    d, i = self.lookup_tree.query([row, col])
                    self.getNewImage(i)
